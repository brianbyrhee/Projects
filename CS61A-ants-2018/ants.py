        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    insect.contain_ant(self.ant) = self.ant
                    self.ant = insect
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        position = self.place
        return position.ant is not None and position.ant.blocks_path
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while position != hive:
            if position.bees and self.min_range <= spaces and spaces <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        position = self.place
        dummy_bees = list(position.bees)
        self.reduce_armor(self.amount)
        if self.armor <= 0:
        	self.place.ant = None
        	for i in dummy_bees:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            bee_var = random_or_none(position.bee)
            if bee_var != None:
                return self.eat_bee(bee_var)
        # END Problem 6
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return not other.is_container and self.contain_ant == None
        # END Problem 9
    ...
    def action(self, colony):
        ...
        position = self.place
        if self.armor <= 0:
            position.ant = self.contained_ant
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self_impostor:
            ...
            while position.exit:
                curr_ant = position.exit.ant
                #if square has ant; we keep track of doubled damage by a helper list
                if curr_ant and curr_ant not in self.doubled_ants:
                    curr_ant.damage *= 2
                    self.doubled_ants.append(curr_ant)

                # edge case for multiple ant(s)
                if isinstance(curr_ant, BodyguardAnt) and curr_ant.contained_ant and curr_ant.contained_ant not in self.doubled_ants:
                    curr_ant.contained_ant.damage *= 2
                    self.doubled_ants.append(curr_ant.contained_ant)
